...
if st.session_state.df is None:
    ...
    st.sidebar.header("Filters")
        # Keep track if we need to reset
    if "need_date_reset" not in st.session_state:
        st.session_state.need_date_reset = False
    if "need_all_reset" not in st.session_state:
        st.session_state.need_all_reset = False

    # Use a form to apply all filters together
    with st.sidebar.form(key="filter_form"):
        # Widgets inside the form - use the reset flags to set initial values
        keywords = st.text_input(
            "Search Question", 
            value="" if st.session_state.need_all_reset else st.session_state.get("keywords", ""),
            key="keywords"
        )
        cat = st.selectbox(
            "Category", 
            ["All"] + list(df["category"].unique()), 
            index=0 if st.session_state.need_all_reset else None,
            key="cat"
        )
        prod = st.selectbox(
            "Product Name", 
        ...
        # For date inputs, use the reset flags
        min_date = df["created_at"].min().date()
        max_date = df["created_at"].max().date()
        
        if st.session_state.need_date_reset or st.session_state.need_all_reset:
            date_min = st.date_input("Start Date", min_date, key="date_min")
            date_max = st.date_input("End Date", max_date, key="date_max")
        else:
            date_min = st.date_input("Start Date", st.session_state.get("date_min", min_date), key="date_min")
            date_max = st.date_input("End Date", st.session_state.get("date_max", max_date), key="date_max")
            
        # For the slider, reset to full range if needed
        if st.session_state.need_all_reset:
            dup_range = st.slider(
                "Duplicates", 0, int(df["duplicate_count"].max()), 
            ...
            dup_range = st.slider(
                "Duplicates", 0, int(df["duplicate_count"].max()),
                st.session_state.get("dup_range", (0, int(df["duplicate_count"].max()))),
                key="dup_range"
            )
    ...
    # Handle resets - just set flags here instead of modifying session state
    if reset_all:
        st.session_state.need_all_reset = True
        st.rerun()
    elif reset_dates:
        st.session_state.need_date_reset = True
        st.rerun()

    # Clear the reset flags after they've been used
    if st.session_state.need_all_reset:
        st.session_state.need_all_reset = False
    if st.session_state.need_date_reset:
        st.session_state.need_date_reset = False
...
def process_duplicates_df(df: pd.DataFrame, group_cols: list):
    """
    Process duplicates based on group_cols. Returns deduped_df, dropped_df.
    """
    ...
    # Normalize text
    for col in ['question', 'answer', 'details']:
        df[col] = df[col].str.strip().str.lower()

    # Compute duplicate counts
    dup_counts = df.groupby(group_cols).size().reset_index(name='duplicate_count')
    temp = df.merge(dup_counts, on=group_cols, how='left')
    temp = temp.sort_values('created_at', ascending=False)

    # Deduplicate
    deduped = temp.drop_duplicates(subset=group_cols, keep='first')
    dropped = temp.loc[~temp.index.isin(deduped.index)]

    return deduped, dropped


def find_matches_with_existing(new_df: pd.DataFrame, existing_df: pd.DataFrame, group_cols: list):
    """
    Compare new_df to existing_df on group_cols, return unique_to_new, matches.
    """
    def normalize(df: pd.DataFrame):
        df2 = df.copy()
        for col in ['question', 'answer', 'details']:
            if col in df2.columns:
                df2[col] = df2[col].fillna('').astype(str).str.strip().str.lower()
        return df2

    a = normalize(new_df)
    b = normalize(existing_df)
    on = [c for c in group_cols if c in a.columns]
...
GROUP_COLS_MAP = {
    'pqad': ['product_name', 'question', 'answer', 'details'],
    'pqa':  ['product_name', 'question', 'answer'],
    'pqd':  ['product_name', 'question', 'details'],
    'pq':   ['product_name', 'question']
}

# Ensure existing data is loaded
if 'df' not in st.session_state:
    st.error("No existing QnA data found in session_state['df'].")
else:
    existing_df = st.session_state.df

    st.sidebar.markdown("---")
    st.sidebar.header("Upload & Preprocess QnA File")

    # Dynamic uploader key for reset
    upload_key = "qna_file"
    if st.session_state.pop("file_upload_canceled", False):
        upload_key += "_reset"

    with st.sidebar.expander("Select File", expanded=True):
    ...
    if uploaded:
        try:
            ...
            st.subheader(f"📥 Loaded: {uploaded.name}")
            st.write(f"Raw rows: {len(raw_df)}")

            # Strategy selection
            strategy = st.sidebar.selectbox(
                "Deduplication Strategy",
                options=list(GROUP_COLS_MAP.keys()),
                format_func=lambda x: {
                    'pqad': "Prod+Q+A+Details",
                    'pqa':  "Prod+Q+A",
                    'pqd':  "Prod+Q+Details",
                    'pq':   "Prod+Q"
                }[x],
            ...
            group_cols = GROUP_COLS_MAP[strategy]
            suffix = datetime.now().strftime('%Y%m%d_%H%M%S')

            # Process duplicates
            cleaned_df, dropped_df = process_duplicates_df(raw_df, group_cols)

            # Download dropped duplicates
            if not dropped_df.empty:
                st.sidebar.download_button(
                    "📥 Download Dropped Duplicates",
                    data=dropped_df.to_csv(index=False).encode('utf-8'),
                    file_name=f"dropped_{suffix}.csv",
                    mime="text/csv"
                )

            # Compare with existing
            unique_entries, matches = find_matches_with_existing(
                cleaned_df, existing_df, group_cols
            )

            # Show merge analytics
            st.markdown("### 📊 Merge Analysis")
            m1, m2, m3 = st.columns(3)
            m1.metric("Cleaned Rows", len(cleaned_df))
            m2.metric("To Merge", len(unique_entries))
            m3.metric("Existing Matches", len(matches))

            # Sidebar merge controls
            with st.sidebar.expander("Merge Actions", expanded=True):
                c1, c2 = st.columns(2)
                if c1.button("✅ Proceed with Merge"):
                    merged_df = pd.concat([existing_df, unique_entries], ignore_index=True)
                    st.session_state.df = merged_df
                    st.success(f"Merged {len(unique_entries)} new entries.")
                    csv_data = merged_df.to_csv(index=False).encode('utf-8')
                    st.download_button(
                        "📥 Download Full Merged QnA",
                        data=csv_data,
                        file_name=f"merged_full_{suffix}.csv",
                        mime="text/csv"
                    )
                    st.button("Continue", on_click=st.rerun)
                if c2.button("❌ Cancel"):
                    st.session_state["file_upload_canceled"] = True
                    st.rerun()

            # Main page previews
            st.markdown("#### ✅ Entries to Merge")
            with st.expander("View Unique Entries", expanded=False):
                st.dataframe(unique_entries.head(20))

            if not matches.empty:
                st.markdown("#### ⚠️ Existing Matches")
                with st.expander("View Sample Matches", expanded=False):
                    st.dataframe(matches.head(10))

            # ─── Additional File Stats & Graphs ───────────────────────────────
            st.markdown("### 📈 Uploaded File Analytics")
            # Summary metrics
            s1, s2, s3 = st.columns(3)
            s1.metric("Raw Rows", len(raw_df))
            s2.metric("Cleaned Rows", len(cleaned_df))
            s3.metric("Dropped Dups", len(dropped_df))

            # Bar chart: product_id count
            if 'product_name' in cleaned_df.columns:
                prod_counts = cleaned_df['product_name'].value_counts()
                st.bar_chart(prod_counts)

            # Time series: created_at distribution
            if 'created_at' in cleaned_df.columns:
                ts = cleaned_df.set_index('created_at').resample('D').size()
                st.line_chart(ts)
